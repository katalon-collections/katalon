...
The backend search endpoint accepts the portal's shape. It parses the comma-separated `facets` parameter into a list of metadata fields, collects every query parameter whose name starts with `meta_`, removes that prefix for `extra_filters`, and passes both values into `search_service.search()` [@search-api]. Anonymous callers are restricted to public records when no explicit status filter is supplied [@search-api].
