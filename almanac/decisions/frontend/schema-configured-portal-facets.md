...
The portal search page consumes that public config. On mount, it calls `api.portal.config()`, stores `facet_fields`, and builds the `/v1/search` `facets` query parameter from the active record type's configured fields or from all configured fields when no type filter is active [@portal-search]. It renders categorical metadata facets from response keys named `meta_<field>` and numeric facets from `numeric_facets`, labels them from the public schema in the active language with German/English fallback, and translates Boolean facet values as Yes/No in the active language. It writes selections back into the URL as repeated `meta_<field>=<value>` parameters or inclusive `range_<field>_from` and `range_<field>_to` bounds. Range sliders commit their local value only when released or blurred, preventing a request for every intermediate position [@portal-search].
