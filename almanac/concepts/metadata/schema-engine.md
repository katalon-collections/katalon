...
sources:
  ...
  - id: schema-doc
    type: web
    url: https://katalon-collections.github.io/katalon-docs/administration/schema/
---
...
Runtime behavior is stricter than a plain JSON editor. The schema API rejects unknown target types, verifies subtype references, blocks nested group fields, validates authority and vocabulary settings, rejects unknown `pid_provider` values on pid fields (`dnb_urn` or `ark`), and prevents deletion of the system `label` field [@schema-api]. Deletion is a soft delete through `is_deleted`, so old JSONB values can remain stored while the field disappears from normal schema reads [@schema-api].
...
Primary records keep cataloguing data in a JSONB column named `metadata` on their ORM tables, exposed in Python as `metadata_` [@models]. `validate_metadata` loads active top-level fields for the record type and optional subtype, checks required values, repeatability, text regexes, PID and URL structures, relation structures, relation target existence, authority entry shape and source matching, group instances, and vocabulary-term field constraints [@schema-service] [@schema-service-tests]. `prepare_metadata` applies configured defaults for text, vocab, vocab-free, date, and number fields, preserves or removes locked fields depending on editor permissions, and protects PID fields unconditionally: stored PID values are kept and client-supplied PID input is dropped, so PID fields can only be written by the mint endpoints and publish auto-minting, regardless of role [@schema-service] [@pid-service]. `url` fields store `{value, label}` objects where `value` is a full http(s) URL and `label` is an optional link title; the importer coerces plain URL strings into that shape.
