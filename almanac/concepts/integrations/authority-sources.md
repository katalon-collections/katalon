...
Admins toggle sources under Settings → Normdatenquellen, which lists every built-in source (merging DB overrides with defaults for sources that have no DB row yet), shows an enable/disable switch per row, and offers a "Verbindung testen" button that issues a source-specific live search. The displayed test request is linked to the source's public search, and success or the raw error appears inline [@settings-screen]. GeoNames API status payloads, such as a missing account or exhausted credits, are reported as errors rather than misleading empty result lists; Getty TGN and AAT use HTTPS for SPARQL and detail lookups [@geonames-adapter] [@tgn-adapter] [@aat-adapter]. Toggling calls `PATCH /authorities/{id}`, which upserts an `authority_sources` row (creating one with default label/adapter class on first toggle) and invalidates the registry cache so the change takes effect immediately [@authority-api] [@authority-service]. The endpoint requires the `admin` role [@authority-api].
