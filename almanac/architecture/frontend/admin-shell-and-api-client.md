...
`AppShell` initializes the admin state from `window.location.hash`, turns hashes such as `#form/<id>` into a `route` and optional `editId`, and updates history with `pushState` when a user navigates [@app-shell]. The shell does not use React Router; it uses a switch over route ids to render list, form, schema, vocabulary, importer, audit, settings, user, static page, OAI set, subtype, and banner screens [@app-shell].

The sidebar is a declarative navigation list that maps ids to labels, icons, active-route aliases, and role restrictions [@sidebar]. Admin-only groups and items check the decoded token role before rendering, so route visibility is handled at the shell-navigation layer while backend endpoints still enforce permissions [@sidebar] [@api-client].
