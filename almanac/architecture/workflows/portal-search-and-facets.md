...
`SearchPage` reads `q`, `type`, `status`, `page`, repeated `meta_*`, `range_<field>_from`, `range_<field>_to`, `rel_entity`, `rel_place`, and `rel_occurrence` parameters from `useSearchParams()` [@search-page]. It turns those values into a request to `/v1/search`, including `page_size=20` and a comma-separated `facets` list when configured metadata facet fields are available [@search-page]. Repeated values of one metadata field are combined as OR; filters from different metadata fields remain conjunctive. Number limits are inclusive, may be open on either side, and reject non-finite values or an inverted range [@search-page] [@search-api].
...
Search results return categorical `facets` as named buckets with `value` and `count`, plus `numeric_facets` with `min` and `max` bounds, matching the portal client response types [@portal-client]. `SearchPage` renders type and status only when enabled in `_system`; active URL filters do not override that configuration. A facet's reset action appears below its values only while that facet is active. Configured number fields render precise native From/To inputs and one shared two-handle range track; unfiltered inputs display the aggregation's min/max without adding a URL filter, and remain available for exact values and accessibility [@search-page]. Other configured metadata panels come from `meta_<field>` aggregation names. Their headings use the public schema label for the active portal language, falling back through German, English, and finally the internal field name [@search-page]. Each aggregation excludes its own active filter while retaining all others, so categorical counts and numeric bounds remain useful while refining [@search-service]. Inherited fields use the same repeated `meta_` URL and backend filter path as direct metadata facets; the field key itself carries the `inherited_<target_type>_<field>` prefix, so the portal can label it as a linked-record facet without adding another query parameter family [@search-page].

Facet value sort order and the initially visible value count are portal-wide `PortalConfig` settings (`facet_sort`: `count` or `alpha`; `facet_initial_count`, default 10), editable in the settings screen's Facetten → Anzeige card and threaded through `portal_public.py` into `search_service.search()` and `elasticsearch.search_documents()` [@settings-screen] [@portal-api] [@search-service]. The Elasticsearch `terms` aggregation for each `meta_<field>` fetches up to 100 buckets with the configured `order`, decoupled from `facet_initial_count` so "show more" can reveal further values already present in the response without a second round-trip [@search-service]. `FacetPanel` renders only the configured initial slice plus any already-active values from beyond that slice (so a selected value never silently disappears), and exposes "show more"/"show less" once more values exist than are shown [@search-page]. This configuration is global, not per field or per record type; a per-field override via `FieldDefinition.settings` was deliberately left out as unneeded for now.
...
For an advanced query, the selected result type replaces the ordinary type filter and the type facet is hidden. Metadata, numeric range, and relation facets remain usable and are included in the advanced POST body [@search-page] [@portal-api].
...
Each result row links by record type: Entities go to `/entities/<id>`, Places to `/places/<id>`, Occurrences to `/occurrences/<id>`, and Objects to `/objects/<id>` [@search-page]. Every row also carries a small type badge next to the title, independent of the admin-configured `subtitle_fields`, so the record type stays visible even when a portal's subtitle configuration omits it [@search-page]. Before following the link, the portal saves the current search URL so detail pages can offer a return path to the same result state [@search-page].

Object results get thumbnail URLs opportunistically. After search returns, the page loads media for object hits, chooses the primary ready file or first ready file, and builds the public JPEG-thumbnail endpoint through `mediaThumbnailUrl` [@search-page] [@portal-client]. Object rows retain their thumbnail area when no ready media exists; entity, place, and occurrence rows render no media column. That keeps search result cards lightweight: the search API returns record summaries and facets, while the object media endpoint supplies thumbnails only where needed.
