...
The global audit endpoint filters by record type, record id, user id, action, and limit, then joins users for display names [@audit-api]. It also resolves record labels per type from the current record tables: object idno or title, entity name, place name, occurrence title, and procedure idno or reference number [@audit-api]. Per-record audit routes in record endpoint modules call the same `list_audit_log` helper with their fixed type and id [@objects-api] [@procedures-api].

## Changed Field Payloads

Create and delete actions often write an empty `changed_fields` object, while update actions store old and new values chosen by each endpoint [@audit-service] [@objects-api] [@procedures-api]. Object updates record old idno, object type, collection status, status, and metadata, then record new object type, collection status, status, and metadata [@objects-api]. Procedure updates record old scalar fields and metadata, then store the validated `ProcedureRead` output as the new state [@procedures-api].

Publishing writes an action named `publish` with `changed_fields` set to the new public status through the publish service, and procedure completion writes one audit entry for the procedure plus one object update entry for every linked object whose collection status changes [@procedures-api]. Those audit rows describe what happened; they are not the mechanism that restores state.

## Snapshots

`RecordSnapshot` stores `record_type`, `record_id`, `label`, JSON `snapshot`, optional `created_by`, and `created_at` [@models]. Object snapshot creation copies idno, object type, collection status, status, and metadata; procedure snapshot creation serializes `ProcedureRead`, including dates and metadata [@objects-api] [@procedures-api]. Other collection record types follow the same smaller pattern for their subtype, status, and metadata fields [@entities-api] [@places-api] [@occurrences-api].

Snapshot listing queries by the fixed record type and id and orders newest first [@objects-api] [@procedures-api]. Restoring an object snapshot copies back only keys present in the snapshot, including idno, status, object type, collection status, and metadata, then flushes the session [@objects-api]. Restoring a procedure snapshot copies procedure type, status, reference number, metadata, and date fields after converting date strings back to `date` objects [@procedures-api].

Restore endpoints do not write audit entries or increment version in the code shown here [@objects-api] [@procedures-api]. A future change that needs restore events to appear in audit history should add explicit `log_change` calls and version increments in each restore endpoint rather than relying on the snapshot row itself.

## Version Boundary

Primary record models carry an integer `version` column with default `1`, and update endpoints increment it after a successful optimistic-locking check [@models] [@objects-api] [@procedures-api]. The optimistic-locking tests define the core contract: missing `If-Match` skips the check, a matching version passes, and a stale version raises `409` with `{"error": "version_conflict", "current_version": <value>}` [@optimistic-tests].
