...
class StorageDeleteError(Exception):
    """One or more storage keys in a ``delete()`` call could not be removed.

    ``failures`` maps each failing key to a human-readable reason. Keys not
    present in ``failures`` were deleted successfully (or were already
    missing, which both backends treat as already-deleted) — a batch is
    never all-or-nothing, so callers that need per-object retry state (see
    ``katalon.services.media_deletion_service``) can tell exactly which
    objects still need another attempt.
    """

    def __init__(self, failures: dict[str, str]) -> None:
        self.failures = failures
        super().__init__(f"Failed to delete {len(failures)} storage object(s): {failures}")
...
class StorageBackend(Protocol):
    ...
    async def delete(self, *keys: str) -> None:
        """Delete objects; a key that is already missing counts as deleted
        (idempotent — safe to retry after a crash or a previous partial
        failure). Raises StorageDeleteError listing exactly the keys that
        could not be removed when any object-level failure occurs; every
        other key in the call was still deleted.
        """
        ...
...
class LocalStorage:
    ...
    async def delete(self, *keys: str) -> None:
        ...
        if failures:
            raise StorageDeleteError(failures)
...
class S3Storage:
    ...
    async def delete(self, *keys: str) -> None:
        ...
        if errors:
            failures = {
                str(err.get("Key", "?")): f"{err.get('Code', 'Error')}: {err.get('Message', '')}"
                for err in errors
            }
            logger.error("S3 DeleteObjects reported per-object failure(s): %s", failures)
            raise StorageDeleteError(failures)
