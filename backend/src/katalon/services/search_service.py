...
def _extract_display_value(val: Any) -> str | None:
    ...
    if isinstance(val, dict):
        return (
            str(val["label"]) if "label" in val else (str(val["value"]) if "value" in val else None)
        )
    if val is not None:
        return str(val)
    return None
...
def _build_doc(
    ...
    idno_value = getattr(record, "idno", None) or ""
    related_text = " ".join(
        name
        for key in (
            "related_entities",
    ...
    for extra in (idno_value, related_text):
        if extra:
    ...
    if record_type == "object":
        ...
        if media_info:
            ...
            if media_info.get("media_width") is not None:
                doc["media_width"] = media_info["media_width"]
            if media_info.get("media_height") is not None:
                doc["media_height"] = media_info["media_height"]
    if export_context is not None:
        doc["export_context"] = export_context
    # Build facet_all_* fields for ALL public metadata fields (not just those
    # marked is_facet), so toggling a facet never requires a full reindex:
    # which fields may be aggregated is enforced at query time against the
    # is_facet whitelist in the public search endpoints.
    ...
    for field_name, val in md.items():
        ...
        if label:
            doc[f"facet_all_{field_name}"] = label
        if field_types.get(field_name) == "number":
            ...
            for value in _advanced_scalar_values(val):
                try:
                    ...
                    continue
            if values:
                doc[f"number_facet_all_{field_name}"] = values if len(values) > 1 else values[0]
    ...
    for rel in relations:
        ...
        if rec:
            ...
            title = _extract_title(
                filter_public_metadata(rec.metadata_ or {}, public_fields, subtype)
            )
            if not title:
...
async def build_index_doc(record_type: str, record: Any, db: Any = None) -> dict[str, Any]:
    ...
    if db is not None:
        ...
        if record_type == "object":
            media_info = await _load_primary_media(record.id, db)
...
async def _load_primary_media(object_id: Any, db: Any) -> dict[str, Any] | None:
    """Load the object's public, ready primary image for the search document.

    Portal result lists render thumbnails straight from the search response, so the
    primary media id and its dimensions travel with the ES document. Selection mirrors
    the portal media endpoint: ready + public + image, primary first.
    """
    from sqlalchemy import select
    ...
    for media in result.scalars().all():
        if media_category(media.mime_type) != "image":
            continue
        return {
...
async def index_record(record_type: str, record: Any, db: AsyncSession) -> None:
    """Build the ES document and defer indexing/RDF-sync dispatch until ``db`` commits.

    ``after_commit`` only records the call; it is turned into a real
    ``enqueue()`` by ``katalon.database.get_db`` (or, for Celery-task-owned
    sessions, an explicit ``run_after_commit_hooks(db)`` call) once this
    request's/task's transaction has actually committed (#392) — a worker
    must never see this record before it is durably visible in Postgres.
    """
    ...
    after_commit(db, index_record_task, record_type, str(record.id), doc)
    if settings.oxigraph_enabled:
        after_commit(db, sync_rdf_record_task, record_type, str(record.id))


async def remove_record(record_id: UUID, db: AsyncSession, record_type: str | None = None) -> None:
    """Defer ES/RDF removal until ``db`` commits — see ``index_record``."""
    from katalon.config import settings
    from katalon.workers.enqueue import after_commit
    from katalon.workers.index_tasks import remove_record_task
    from katalon.workers.rdf_tasks import remove_rdf_record_task

    after_commit(db, remove_record_task, str(record_id))
    if settings.oxigraph_enabled:
        after_commit(db, remove_rdf_record_task, record_type, str(record_id))
...
async def search(
    ...
    for h in hits.get("hits", []):
        ...
        values = {
            name: source[f"facet_all_{name}"]
            for name in wanted
            if name not in ("record_type", "status", "idno") and f"facet_all_{name}" in source
        }
        if "idno" in wanted and source.get("idno"):
            values["idno"] = source["idno"]
        if values:
    ...
    for agg_key, agg_val in aggs.items():
        ...
        facets[agg_key] = [{"value": b["key"], "count": b["doc_count"]} for b in buckets]
...
async def search_admin_data(
    ...
    vocab_terms = (
        await db.execute(
            ...
            .join(Vocabulary)
            .where(
                _contains(q, VocabularyTerm.term, cast(VocabularyTerm.label, Text), Vocabulary.name)
            )
            .limit(limit)
        )
    ).all()
    ...
    groups: list[tuple[Any, str, str, Any]] = [
        (
            select(User).where(_contains(q, User.email)),
            "user",
            "users",
            lambda row: (row.email, row.role, None),
        ),
        (
            select(Vocabulary).where(_contains(q, Vocabulary.name)),
            "vocabulary",
            "vocab",
            lambda row: (row.name, None, row.name),
        ),
        (
            select(StaticPage).where(
                _contains(
                    q, StaticPage.slug, cast(StaticPage.title, Text), cast(StaticPage.content, Text)
                )
            ),
            "page",
            "pages",
            lambda row: (
                row.title.get("de") or row.title.get("en") or row.slug,
                row.slug,
                row.slug,
            ),
        ),
        (
            select(OAISet).where(
                _contains(
                    ...
                    OAISet.set_name,
                    OAISet.filter_q,
                    cast(OAISet.filter_metadata, Text),
                )
            ),
            "oai_set",
            "oai-sets",
            lambda row: (row.set_name, row.set_spec, None),
        ),
        (
            select(FieldDefinition).where(
                _contains(q, FieldDefinition.name, cast(FieldDefinition.label, Text))
            ),
            "schema_field",
            "schema",
            lambda row: (
                row.label.get("de") or row.label.get("en") or row.name,
                row.target_type,
                row.target_type,
            ),
        ),
        (
            select(RecordSubtype).where(
                _contains(
                    q,
                    RecordSubtype.name,
                    RecordSubtype.description,
                    cast(RecordSubtype.label, Text),
                )
            ),
            "subtype",
            "subtypes",
        ...
        (
            select(FormVariant).where(
                FormVariant.is_deleted.is_(False),
                _contains(q, FormVariant.name, cast(FormVariant.label, Text)),
            ),
            "form_variant",
            "form-variants",
            lambda row: (
                row.label.get("de") or row.label.get("en") or row.name,
                row.target_type + (f".{row.target_subtype}" if row.target_subtype else ""),
                row.target_type + (f".{row.target_subtype}" if row.target_subtype else ""),
            ),
        ),
        (
            select(Banner).where(_contains(q, Banner.message)),
            "banner",
            "banners",
            lambda row: (row.message, None, None),
        ),
        (
            select(AuthoritySource).where(_contains(q, AuthoritySource.id, AuthoritySource.label)),
            "authority_source",
            "settings",
            lambda row: (row.label, row.id, "authorities"),
        ),
    ]
