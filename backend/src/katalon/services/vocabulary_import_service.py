...
def _find_parent_cycles(terms: list[ImportTerm], existing_terms: list[VocabularyTerm]) -> set[str]:
    """Return vocabulary term names participating in the imported parent graph's cycles."""
    names_by_id = {term.id: term.term for term in existing_terms}
    parents = {term.term: names_by_id.get(term.parent_id) for term in existing_terms}
    parents.update({item.term: item.parent_term for item in terms})
    cycles: set[str] = set()
    for start in parents:
        positions: dict[str, int] = {}
        path: list[str] = []
        current: str | None = start
        while current is not None and current in parents and current not in cycles:
...
async def import_vocabulary_terms(
    ...
    combined_term_names = (
        ...
        if strategy == "replace"
        else set(existing_by_term) | {t.term for t in terms}
    )
    ...
    cyclic_terms = _find_parent_cycles(valid_terms, existing_terms)
    if cyclic_terms:
        for item in valid_terms:
            if item.term in cyclic_terms:
                errors.append(
                    {
                        "row": item.row,
                        "message": f"Hierarchiezyklus für '{item.term}'",
                    }
                )
        valid_terms = [item for item in valid_terms if item.term not in cyclic_terms]
