...
from katalon.services.relation_service import sync_schema_relations, validate_relation_endpoint
...
async def _load_field_definition(
    ...
    result = await db.execute(
        select(FieldDefinition).where(
            ...
            or_(
                FieldDefinition.target_subtype.is_(None), FieldDefinition.target_subtype == subtype
            ),
        )
    )
...
async def _apply_field_operation(
    ...
    metadata = deepcopy(record.metadata_)
    ...
    errors = await validate_metadata(
        db, record_type, metadata, subtype, existing=record.metadata_, skip_required=True
    )
    if errors:
    ...
    status_val = getattr(record, "status", None)
    diff = diff_fields(
        {"status": status_val, "metadata": old_metadata},
        {"status": status_val, "metadata": metadata},
    )
    if diff:
        diff["batch_job_id"] = str(batch_job_id)
        await log_change(
...
async def _apply_relation_operation(
    ...
    applicability_error = await validate_relation_type_applicability(
        db, record_type, to_type, relation_type
    )
    if applicability_error:
        raise ValueError(applicability_error)

    if operation_type == "add_relation":
        target_error = await validate_relation_endpoint(db, to_type, to_id)
        if target_error:
        ...
        try:
            async with db.begin_nested():
                ...
                if record is None or (
                    hasattr(record, "deleted_at") and record.deleted_at is not None
                ):
                    raise ValueError("Datensatz nicht gefunden.")
                await enforce_not_blocked(db, record_type, record.id, current_user)
                await enforce_not_locked(db, record_type, record.id, current_user)
                ...
                if op_type == "set_status":
                    changed = await _apply_status_change(
                        db, record, record_type, operation.value, current_user.id, batch_job_id
                    )
            ...
            logger.warning(
                "Batch operation failed for %s/%s: %s", record_type, record_id, exc, exc_info=True
            )
