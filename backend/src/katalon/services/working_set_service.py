...
async def get_working_set_detail(
    ...
    if raw_items:
        ...
        # Only surface items the *current viewer* may read: a shared working
        # set must not leak draft/internal records to a recipient without
        # record-read permission on the type, even if the owner could see
        # them (#389).
        visibility_user = (
            user if await has_record_permission(db, user, ws.record_type, "read") else None
        )
        ...
        for it in raw_items:
            ...
            if (
                rec is not None
                and visibility_user is None
                and hasattr(model, "status")
                and getattr(rec, "status", None) not in PUBLIC_STATUSES
            ):
                # Record exists but this viewer may not read it — drop it
                # from the resolved set entirely rather than masking it.
                continue
