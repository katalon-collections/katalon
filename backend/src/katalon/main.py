...
app.include_router(preservation.router, prefix="/v1", dependencies=_authenticated)
...
# #396: default Celery/Redis-broker queue name — no custom queue is
# configured (see workers/celery_app.py), so this is the literal Redis list
# key holding pending tasks.
_CELERY_QUEUE_NAME = "celery"


def _check_redis() -> dict[str, object]:
    """Ping Redis and, in the same connection, read two more signals it makes
    cheap to obtain: the default queue's backlog length and the Beat/Worker
    heartbeat key.

    The heartbeat is a dead-man's-switch (#396): Beat enqueues
    ``katalon.heartbeat`` every 2 minutes and a Worker refreshes the key
    (10-minute TTL) when it executes the task — see
    ``workers/health_tasks.py`` for why a missing key deliberately does not
    distinguish "Beat is down" from "no Worker is consuming the queue" from
    "Redis is unreachable": all three are outage conditions needing the same
    first response.
    """
    import redis as redis_lib

    from katalon.workers.health_tasks import HEARTBEAT_KEY
    ...
    try:
        ...
        return {"redis": error, "queue_backlog": None, "beat_heartbeat": error}

    queue_backlog = int(client.llen(_CELERY_QUEUE_NAME))
    stale_msg = "error: stale or missing (no heartbeat in last 10m)"
    beat_heartbeat = "ok" if client.exists(HEARTBEAT_KEY) else stale_msg
    return {"redis": "ok", "queue_backlog": queue_backlog, "beat_heartbeat": beat_heartbeat}


async def _check_cantaloupe() -> str:
    """Cheap reachability check against Cantaloupe's IIIF Image API root.
    Short timeout so a slow/hanging Cantaloupe can't stall /health.
    """
    import httpx

    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            resp = await client.get(f"{settings.cantaloupe_url}/iiif/3")
        if resp.status_code < 400:
            return "ok"
        return f"error: HTTP {resp.status_code}"
    except Exception as exc:
        logger.warning("Health check: cantaloupe unreachable", exc_info=True)
        return f"error: {exc.__class__.__name__}"


async def _check_oxigraph() -> str:
    from katalon.integrations.oxigraph import get_oxigraph_client

    client = get_oxigraph_client()
    try:
        return "ok" if await client.health() else "error: unreachable"
    except Exception as exc:
        logger.warning("Health check: oxigraph unreachable", exc_info=True)
        return f"error: {exc.__class__.__name__}"
    finally:
        await client.close()
...
def _backup_age_hours() -> float | None:
    """Hours since the last successful backup, per the UTC ISO-8601 marker
    file ``docker/backup.sh`` writes after each successful run (#396).

    Returns ``None`` unless an operator has optionally mounted the backup
    volume read-only into the api container — not the default topology, see
    docs/04_produktion.md#monitoring for the recommended host-side check
    that needs no compose change at all.
    """
    marker = Path("/backups/.last_success")
    try:
        raw = marker.read_text().strip()
    except FileNotFoundError:
        return None
    except Exception:
        logger.warning("Health check: backup marker unreadable", exc_info=True)
        return None

    try:
        last_success = datetime.strptime(raw, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=UTC)
    except ValueError:
        logger.warning("Health check: backup marker malformed: %r", raw)
        return None
    return (datetime.now(UTC) - last_success).total_seconds() / 3600


@app.get("/health", tags=["system"])
async def health(response: Response) -> dict[str, object]:
    """Readiness check: verifies DB, Elasticsearch, Redis, Cantaloupe and
    (if enabled) Oxigraph are reachable, plus Celery Beat/Worker liveness via
    a heartbeat key (see workers/health_tasks.py).

    Returns 503 if any entry in ``checks`` is not "ok" so load balancers can
    fail over. ``queue_backlog`` and ``backup_age_hours`` are informational
    only and never affect the status code — see
    docs/04_produktion.md#monitoring for suggested alerting thresholds.
    """
    ...
    checks["redis"] = cast(str, redis_result["redis"])
    checks["beat_heartbeat"] = cast(str, redis_result["beat_heartbeat"])
    queue_backlog = redis_result["queue_backlog"]

    checks["cantaloupe"] = await _check_cantaloupe()

    if settings.oxigraph_enabled:
        checks["oxigraph"] = await _check_oxigraph()
