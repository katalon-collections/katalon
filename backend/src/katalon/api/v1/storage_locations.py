...
async def list_storage_locations(
    db: DBDep,
    _: User = require_record_permission("storage_location", "read"),
...
async def update_storage_location(
    ...
    if not loc:
        raise HTTPException(status_code=404, detail="Lagerort nicht gefunden")
    require_version(loc.version, if_match)
    ...
    errors = await validate_metadata(
        db, "storage_location", metadata, storage_location_type, existing=loc.metadata_
    )
    if errors:
        raise HTTPException(status_code=422, detail=errors)
...
@router.post(
    "/{loc_id}/restore",
    response_model=StorageLocationRead,
    summary="Restore a soft-deleted storage location",
    responses={
        404: {"description": "Storage location not found or not deleted"},
        403: {"description": "Insufficient permissions"},
    },
)
async def restore_storage_location(
    ...
    result = await db.execute(select(StorageLocation).where(StorageLocation.id == loc_id))
    loc = result.scalar_one_or_none()
    if not loc or loc.deleted_at is None:
        raise HTTPException(status_code=404, detail="Lagerort nicht gefunden")
    loc.deleted_at = None
    await log_change(
        db,
...
@router.get(
    ...
    summary="List soft-deleted storage locations",
)
async def list_deleted_storage_locations(
    db: DBDep, current_user: User = require_role("admin")
) -> list[StorageLocation]:
    result = await db.execute(
        select(StorageLocation)
        .where(StorageLocation.deleted_at.is_not(None))
        .order_by(StorageLocation.deleted_at.desc())
    )
...
async def list_storage_location_objects(
    loc_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("storage_location", "read"),
    include_sublocations: bool = True,
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=200),
) -> StorageLocationObjectsResponse:
    if not await has_record_permission(db, current_user, "object", "read"):
        raise HTTPException(status_code=403, detail="Keine Leseberechtigung für Objekte")
...
@router.get(
    ...
    summary="List audit log entries for a storage location",
)
async def list_storage_location_audit_log(
    loc_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("storage_location", "read"),
) -> list[AuditLogRead]:
    ...
    return await list_audit_log(
        db, current_user=current_user, record_type="storage_location", record_id=loc_id, limit=100
    )
