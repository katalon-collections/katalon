...
def _range_filters(request: Request) -> dict[str, tuple[float | None, float | None]]:
    ...
    result = {field: (bounds.get("from"), bounds.get("to")) for field, bounds in values.items()}
    if any(
        lower is not None and upper is not None and lower > upper
        for lower, upper in result.values()
    ):
...
async def search(
    ...
    facets: Annotated[
        str | None, Query(description="Comma-separated metadata fields to aggregate")
    ] = None,
    page: Annotated[int, Query(ge=1)] = 1,
    page_size: Annotated[int, Query(ge=1, le=100)] = 20,
    rel_entity: Annotated[
        str | None, Query(description="Filter objects by related entity name")
    ] = None,
    rel_place: Annotated[
        str | None, Query(description="Filter objects by related place name")
    ] = None,
    rel_occurrence: Annotated[
        str | None, Query(description="Filter objects by related occurrence name")
    ] = None,
    rel_collection: Annotated[
...
async def trigger_reindex_type(target_type: str) -> dict[str, str]:
    if target_type not in RECORD_TYPES:
        from fastapi import HTTPException

        raise HTTPException(
            status_code=422, detail=f"Ungültiger Typ. Erlaubt: {', '.join(sorted(RECORD_TYPES))}"
        )
    from katalon.workers.enqueue import enqueue_or_503
