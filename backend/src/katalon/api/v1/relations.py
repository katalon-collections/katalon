...
async def list_relations(
    ...
    async def _can_read(record_type: str) -> bool:
        if record_type not in readable:
            readable[record_type] = await has_record_permission(db, current_user, record_type, "read")
        return readable[record_type]

    # Only surface relations whose *both* endpoints the caller may read — an
    # authorized endpoint must never leak the type/id/label of an
    # unauthorized counterpart (issue #388).
    visible_relations = [
        rel for rel in relations if await _can_read(rel.from_type) and await _can_read(rel.to_type)
    ]
    labels = await resolve_relation_labels(db, visible_relations)
    return [
