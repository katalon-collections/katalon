...
from katalon.core.visibility import SEARCHABLE_RECORD_TYPES, readable_record_types
...
def _visible_types_clause(readable_types: tuple[str, ...] | None):
    """SQL clause restricting `AuditLog.record_type` to types the caller may read.

    Only the seven role-permissioned record types are governed by a per-type
    read right; other audit domains (`user`, `schema`, `sparql`, lock targets,
    ...) have no record-permission matrix and stay visible to any
    `audit_log` feature holder, unchanged from before this fix. `None` means
    admin/superuser — unrestricted."""
    if readable_types is None:
        return None
    return or_(
        AuditLog.record_type.notin_(SEARCHABLE_RECORD_TYPES),
        AuditLog.record_type.in_(readable_types),
    )


def _type_readable(record_type: str, readable_types: tuple[str, ...] | None) -> bool:
    if readable_types is None or record_type not in SEARCHABLE_RECORD_TYPES:
        return True
    return record_type in readable_types


def _scrub_unreadable_related(
    changed_fields: dict[str, object] | None, readable_types: tuple[str, ...] | None
) -> dict[str, object] | None:
    """Strip relation-partner id/type/label from a diff when the caller may
    not read that related record type — mirrors the relation-list guard
    (issue #388): unauthorized endpoints must not leak via labels or ids."""
    if not changed_fields:
        return changed_fields
    related_type = changed_fields.get("related_record_type")
    if not isinstance(related_type, str) or _type_readable(related_type, readable_types):
        return changed_fields
    return {
        k: v
        for k, v in changed_fields.items()
        if k not in {"related_record_id", "related_record_type", "related_record_label"}
    }
...
async def list_audit_log(
    db: DBDep,
    current_user: User = require_feature("audit_log"),
    record_type: str | None = None,
    record_id: uuid.UUID | None = None,
    user_id: uuid.UUID | None = None,
    action: str | None = None,
    ...
    readable_types = await readable_record_types(db, current_user)
    ...
    visible_clause = _visible_types_clause(readable_types)
    ...
    for log in logs:
        ...
        if (
            isinstance(related_type, str)
            and related_uuid
            and _type_readable(related_type, readable_types)
        ):
    ...
    for log, user_email in rows:
        changed_fields = _scrub_unreadable_related(
            _collapse_diff_values(log.changed_fields), readable_types
        )
