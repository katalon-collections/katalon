...
async def update_collection(
    ...
    await enforce_not_blocked(db, "collection", col.id, current_user)
    await enforce_not_locked(db, "collection", col.id, current_user)
...
async def list_snapshots(
    col_id: uuid.UUID,
    db: DBDep,
    _: User = require_record_permission("collection", "read"),
) -> list[RecordSnapshot]:
    result = await db.execute(
...
async def list_collection_audit_log(
    col_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("collection", "read"),
) -> list[AuditLogRead]:
    from katalon.api.v1.audit import list_audit_log

    return await list_audit_log(
        db, current_user=current_user, record_type="collection", record_id=col_id, limit=100
    )
