...
from katalon.core.concurrency import flush_record, require_version
...
async def list_occurrences(
    ...
    query = apply_sort(
        query.offset((page - 1) * page_size).limit(page_size), Occurrence, sort_by, sort_dir
    )
...
async def create_occurrence(
    ...
    db: DBDep,
    current_user: User = require_record_permission("occurrence", "create"),
) -> Occurrence:
    ...
    await db.flush()
    await log_change(
        db, record_type="occurrence", record_id=occ.id, user_id=current_user.id, action="create"
    )
...
async def get_occurrence(
    ...
    if visibility_user is None:
        return await project_public_record(
            db, OccurrenceRead.model_validate(occ), "occurrence", occ.occurrence_type
        )
    return occ
...
async def update_occurrence(
    ...
    if not data.idno or not data.idno.strip():
        ...
        existing = await db.execute(
            select(Occurrence).where(Occurrence.idno == idno, Occurrence.id != occ_id)
        )
        if existing.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="ID-Nr. bereits vergeben.")
    occurrence_type = normalize_subtype_name(
    ...
    if old["status"] not in PUBLIC_SAVE_STATUSES and data.status in PUBLIC_SAVE_STATUSES:
        try:
            await pid_service.ensure_pids_on_publish(db, "occurrence", occ, current_user.id)
        except pid_service.PidMintError as exc:
            raise HTTPException(
...
async def delete_occurrence(
    ...
    await flush_record(db, occ)
    try:
        await search_service.remove_record(occ.id, db, record_type="occurrence")
    except Exception:
        logger.warning("ES index/remove failed", exc_info=True)
...
async def restore_occurrence(
    ...
    occ.deleted_at = None
    await log_change(
        db, record_type="occurrence", record_id=occ.id, user_id=current_user.id, action="undelete"
    )
    await flush_record(db, occ)
    try:
        await search_service.index_record("occurrence", occ, db)
...
async def list_deleted_occurrences(
    db: DBDep, current_user: User = require_role("admin")
) -> list[Occurrence]:
    result = await db.execute(
        select(Occurrence)
        .where(Occurrence.deleted_at.is_not(None))
        .order_by(Occurrence.deleted_at.desc())
    )
...
@router.post(
    "/{occ_id}/purge",
    status_code=204,
    summary="Permanently delete a soft-deleted occurrence (hard delete)",
    responses={
        404: {"description": "Occurrence not found or not in trash"},
        403: {"description": "Insufficient permissions"},
    },
)
async def purge_occurrence(
    occ_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_role("admin"),
) -> None:
    ...
    ok = await purge_record_now(db, "occurrence", occ_id, current_user.id)
    if not ok:
        raise HTTPException(
...
async def list_snapshots(
    occ_id: uuid.UUID,
    db: DBDep,
    _: User = require_record_permission("occurrence", "read"),
) -> list[RecordSnapshot]:
    result = await db.execute(
...
async def list_occurrence_audit_log(
    occ_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("occurrence", "read"),
) -> list[AuditLogRead]:
    from katalon.api.v1.audit import list_audit_log

    return await list_audit_log(
        db, current_user=current_user, record_type="occurrence", record_id=occ_id, limit=100
    )
