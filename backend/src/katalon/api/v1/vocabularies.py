...
async def _validate_parent(
    ...
    if not vocab.is_hierarchical:
        raise HTTPException(
            status_code=422, detail="Nur hierarchische Vokabulare dürfen übergeordnete Terme haben"
        )
    ...
    if parent is None:
        raise HTTPException(
            status_code=422, detail="Übergeordneter Term gehört nicht zu diesem Vokabular"
        )
    ...
    while current is not None:
        ...
        if current.id == term_id:
            raise HTTPException(
                status_code=422, detail="Ein Term kann nicht sein eigener Nachfahre sein"
            )
        seen.add(current.id)
...
async def list_terms(
    ...
    from_type: str | None = Query(
        None, description="Only terms allowed for this source record type"
    ),
...
async def create_term(vocab_id: uuid.UUID, data: VocabularyTermCreate, db: DBDep) -> VocabularyTerm:
    vocab = await db.get(Vocabulary, vocab_id)
    if not vocab:
        raise HTTPException(status_code=404, detail="Vokabular nicht gefunden")
    ...
    errors = await validate_metadata(db, "vocabulary_term", metadata, str(vocab_id))
    if errors:
        raise HTTPException(status_code=422, detail=errors)
    term = VocabularyTerm(
        **data.model_dump(exclude={"vocabulary_id", "metadata_"})
        | {"vocabulary_id": vocab_id, "metadata_": metadata}
    )
    db.add(term)
    await db.commit()
    return term
...
async def update_term(term_id: uuid.UUID, data: VocabularyTermUpdate, db: DBDep) -> VocabularyTerm:
    ...
    if "metadata_" in dump and dump["metadata_"] is not None:
        prepared_meta = await prepare_metadata(
            db, "vocabulary_term", dump["metadata_"], str(term.vocabulary_id), existing=term.metadata_
        )
        errors = await validate_metadata(
            db, "vocabulary_term", prepared_meta, str(term.vocabulary_id), existing=term.metadata_
        )
        if errors:
            raise HTTPException(status_code=422, detail=errors)
        dump["metadata_"] = prepared_meta
...
class TermUsageResponse(BaseModel):
    term_id: uuid.UUID
    usage_count: int
...
@router.get(
    "/terms/{term_id}/usage",
    response_model=TermUsageResponse,
    dependencies=[require_feature("vocab_terms")],
    summary="Get usage count for a vocabulary term",
    responses={
        404: {"description": "Term not found"},
    },
)
async def get_term_usage(term_id: uuid.UUID, db: DBDep) -> TermUsageResponse:
    result = await db.execute(select(VocabularyTerm).where(VocabularyTerm.id == term_id))
    term = result.scalar_one_or_none()
    if not term:
        raise HTTPException(status_code=404, detail="Term nicht gefunden")
    usage_count = await count_references(db, term.vocabulary_id, term.id)
    return TermUsageResponse(term_id=term.id, usage_count=usage_count)
...
async def delete_term(
    ...
    if replacement_term_id is not None:
        if replacement_term_id == term.id:
            raise HTTPException(
                status_code=422, detail="Ein Term kann nicht auf sich selbst umgehängt werden."
            )
        replacement = await db.get(VocabularyTerm, replacement_term_id)
        if replacement is None or replacement.vocabulary_id != term.vocabulary_id:
            raise HTTPException(
                status_code=422, detail="Ersatzterm gehört nicht zu diesem Vokabular."
            )
...
async def import_terms(
    ...
    if filename.endswith(".csv") or filename.endswith(".tsv"):
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"CSV konnte nicht verarbeitet werden: {exc}"
            ) from exc
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"JSON konnte nicht verarbeitet werden: {exc}"
            ) from exc
    else:
    ...
    if vocab.kind == "relation" and any(term.parent_term for term in terms):
        raise HTTPException(
            status_code=422, detail="Relationstypen dürfen keine übergeordneten Terme haben"
        )
...
async def import_skos_terms(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail=f"SKOS-Datei konnte nicht verarbeitet werden: {exc}"
        ) from exc
