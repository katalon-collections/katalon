...
async def list_procedures(
    ...
    query = apply_sort(
        query.offset((page - 1) * page_size).limit(page_size), Procedure, sort_by, sort_dir
    )
...
async def get_procedure(
    procedure_id: uuid.UUID, db: DBDep, _: User = require_record_permission("procedure", "read")
) -> Procedure:
    proc = (
        await db.execute(select(Procedure).where(Procedure.id == procedure_id))
    ).scalar_one_or_none()
...
async def update_procedure(
    ...
    if not data.idno or not data.idno.strip():
        ...
        if existing.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="ID-Nr. bereits vergeben.")
    await _validate_procedure(data, db, procedure_id, existing=proc.metadata_)
    old = {
        "idno": proc.idno,
...
async def delete_procedure(
    ...
    if related_count > 0 and not force:
        raise HTTPException(
            ...
            detail={
                "detail": (
                    f"Dieser Datensatz ist mit {related_count} anderen Datensätzen verknüpft."
                ),
    ...
    await cleanup_relation_refs(db, "procedure", procedure_id)
    await log_change(
        db,
        record_type="procedure",
        record_id=proc.id,
        ...
        action="delete",
        changed_fields={
            k: v for k, v in {"idno": proc.idno, "title": proc.reference_number}.items() if v
        },
    )
    await db.delete(proc)
    await flush_record(db, proc)
    try:
        await search_service.remove_record(procedure_id, db, record_type="procedure")
    except Exception:
        logger.warning("ES index/remove failed", exc_info=True)
...
async def list_procedure_audit_log(
    procedure_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("procedure", "read"),
) -> list[AuditLogRead]:
    from katalon.api.v1.audit import list_audit_log

    return await list_audit_log(
        db, current_user=current_user, record_type="procedure", record_id=procedure_id, limit=100
    )
