...
async def delete_object(
    ...
    await flush_record(db, obj)
    try:
        await search_service.remove_record(obj.id, db, record_type="object")
    except Exception:
        logger.warning("ES index/remove failed", exc_info=True)
...
async def list_snapshots(
    object_id: uuid.UUID,
    db: DBDep,
    _: User = require_record_permission("object", "read"),
) -> list[RecordSnapshot]:
    result = await db.execute(
...
async def list_object_audit_log(
    object_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("object", "read"),
) -> list[AuditLogRead]:
    from katalon.api.v1.audit import list_audit_log

    return await list_audit_log(
        db, current_user=current_user, record_type="object", record_id=object_id, limit=100
    )
