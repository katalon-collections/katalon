...
async def list_entities(
    ...
    query = apply_sort(
        query.offset((page - 1) * page_size).limit(page_size), Entity, sort_by, sort_dir
    )
...
async def create_entity(
    data: EntityCreate,
    db: DBDep,
    current_user: User = require_record_permission("entity", "create"),
) -> Entity:
    ...
    await db.flush()
    await log_change(
        db, record_type="entity", record_id=entity.id, user_id=current_user.id, action="create"
    )
...
async def get_entity(
    ...
    if visibility_user is None:
        return await project_public_record(
            db, EntityRead.model_validate(entity), "entity", entity.entity_type
        )
    return entity
...
async def update_entity(
    ...
    await enforce_not_blocked(db, "entity", entity.id, current_user)
    await enforce_not_locked(db, "entity", entity.id, current_user)
    if not data.idno or not data.idno.strip():
        ...
        existing = await db.execute(
            select(Entity).where(Entity.idno == idno, Entity.id != entity_id)
        )
        if existing.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="ID-Nr. bereits vergeben.")
    entity_type = normalize_subtype_name(
    ...
    await flush_record(db, entity)
    await sync_schema_relations(db, "entity", entity.id, metadata)
    entity_diff = diff_fields(
        old, {"idno": idno, "entity_type": entity_type, "status": data.status, "metadata": metadata}
    )
    if entity_diff:
        await log_change(
            db,
...
async def delete_entity(
    ...
    await flush_record(db, entity)
    try:
        await search_service.remove_record(entity.id, db, record_type="entity")
    except Exception:
        logger.warning("ES index/remove failed", exc_info=True)
...
async def restore_entity(
    ...
    await log_change(
        db, record_type="entity", record_id=entity.id, user_id=current_user.id, action="undelete"
    )
    await flush_record(db, entity)
    try:
        await search_service.index_record("entity", entity, db)
...
async def list_snapshots(
    entity_id: uuid.UUID,
    db: DBDep,
    _: User = require_record_permission("entity", "read"),
) -> list[RecordSnapshot]:
    result = await db.execute(
...
async def list_entity_audit_log(
    entity_id: uuid.UUID,
    db: DBDep,
    current_user: User = require_record_permission("entity", "read"),
) -> list[AuditLogRead]:
    from katalon.api.v1.audit import list_audit_log

    return await list_audit_log(
        db, current_user=current_user, record_type="entity", record_id=entity_id, limit=100
    )
