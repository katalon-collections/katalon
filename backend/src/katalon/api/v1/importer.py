...
"""CSV, Excel, and XML importer API.

Supports the four primary inventory record types: 'object', 'entity', 'place', and 'occurrence'.
Hierarchical collections, procedure workflows, and internal storage locations are intentionally
excluded from this generic tabular/XML import pipeline.
"""
...
from katalon.core.dependencies import DBDep, has_record_permission, require_feature, require_role
...
# Generic importer supports the 4 primary inventory record types.
# 'collection', 'procedure', and 'storage_location' have dedicated hierarchical
# or workflow requirements and are intentionally excluded.
...
class MappingRequest(BaseModel):
    ...
    record_type: str = Field(
        default="object",
        description="Target record type ('object', 'entity', 'place', 'occurrence').",
    )
...
@router.post(
    ...
    description=(
        "Upload tabular files (CSV, TSV, Excel) or XML files to temporary staging for "
        "importing 'object', 'entity', 'place', or 'occurrence' records."
    ),
    responses={
...
async def upload_file(
    ...
    current_user: User = require_feature("import"),
) -> dict[str, Any]:
    if not files:
...
async def xml_selectors(body: XmlSelectorsRequest, current_user: User = require_feature("import")) -> dict[str, Any]:
...
async def _ensure_import_write_permission(db: AsyncSession, user: User, record_type: str) -> None:
    """Import may create new records or upsert existing ones — require both
    create and update permission for the chosen type, not just the coarse
    `import` feature (#389).
    """
    if not (
        await has_record_permission(db, user, record_type, "create")
        and await has_record_permission(db, user, record_type, "update")
    ):
        raise HTTPException(status_code=403, detail="Keine Schreibberechtigung für diesen Datensatztyp.")

@router.post(
    "/dry-run",
    summary="Preview an import without persisting records",
    description=(
        "Validate mappings, required fields, and schema constraints without persisting records. "
        "Supported record types are 'object', 'entity', 'place', and 'occurrence'."
    ),
    responses={
        403: {"description": "Insufficient permissions"},
...
async def dry_run(body: MappingRequest, db: DBDep, current_user: User = require_feature("import")) -> dict[str, Any]:
    """Validate mappings, required fields, and schema constraints without persisting records.

    Supported record types: 'object', 'entity', 'place', 'occurrence'.
    Collections, procedures, and storage locations are intentionally unsupported.
    """
    if body.record_type not in VALID_TYPES:
        raise HTTPException(status_code=422, detail=f"Ungültiger Typ: {body.record_type}")
    await _ensure_import_write_permission(db, current_user, body.record_type)
...
@router.post(
    ...
    description=(
        "Queue a Celery background task to import staged records. "
        "Supported record types are 'object', 'entity', 'place', and 'occurrence'."
    ),
    responses={
        403: {"description": "Insufficient permissions"},
...
async def run_import(body: ImportRequest, db: DBDep, current_user: User = require_feature("import")) -> dict[str, Any]:
    """Queue a Celery task to import records for 'object', 'entity', 'place', or 'occurrence'.
...
@router.get(
    ...
    description="Return saved import mappings, optionally filtered by record type ('object', 'entity', 'place', 'occurrence').",
...
async def list_import_mappings(
    ...
    """Return saved import mappings, optionally filtered by record type ('object', 'entity', 'place', 'occurrence')."""
...
@router.post(
    ...
    description="Persist an import mapping template for 'object', 'entity', 'place', or 'occurrence'.",
    responses={
        403: {"description": "Insufficient permissions"},
...
async def create_import_mapping(
    ...
    """Persist an import mapping template for 'object', 'entity', 'place', or 'occurrence'."""
    if body.record_type not in VALID_TYPES:
        raise HTTPException(status_code=422, detail=f"Ungültiger Typ: {body.record_type}")
...
@router.post(
    ...
    description=(
        "Create new field definitions on-the-fly for unmapped import columns. "
        "Supported record types are 'object', 'entity', 'place', and 'occurrence'."
    ),
    responses={
        403: {"description": "Insufficient permissions"},
...
async def create_fields(body: CreateFieldsRequest, db: DBDep) -> dict[str, Any]:
    ...
    Supported for 'object', 'entity', 'place', and 'occurrence'.
...
async def cancel_task(task_id: str, current_user: User = require_feature("import")) -> dict[str, Any]:
    r = _get_redis()
...
async def task_status(task_id: str, current_user: User = require_feature("import")) -> dict[str, Any]:
