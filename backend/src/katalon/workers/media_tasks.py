...
async def _best_effort_delete(storage: StorageBackend, *keys: str) -> None:
    """Clean up a just-uploaded object with no MediaFile row yet (validation
    rejected it before the row was created). There is nothing to protect
    here — no committed DB reference exists — so a storage failure is
    logged and swallowed rather than aborting the whole batch import."""
    try:
        await storage.delete(*keys)
    except StorageDeleteError as exc:
        logger.warning("Best-effort cleanup of rejected upload failed: %s", exc.failures)
...
async def _process(media_file_id: uuid.UUID) -> dict[str, Any]:
    ...
    async with _worker_session()() as session:
        ...
        media.status = "ready"
        await session.commit()
        # The object's search document caches primary media id/dimensions – refresh it
        # now that the media left the pending state.
        from katalon.core.models import Object
        from katalon.services.search_service import index_record

        obj = await session.get(Object, media.object_id)
        if obj is not None:
            await index_record("object", obj, session)
        ...
        for file_path, object_id_raw, media_type in planned:
            ...
            existing_filenames = (
                await session.execute(
                    select(MediaFile.filename).where(
                        MediaFile.object_id == object_uuid, MediaFile.deleted_at.is_(None)
                    )
                )
            ).scalars().all()
            ...
            after_commit(session, generate_iiif_tiles, str(file_id))
            created += 1
