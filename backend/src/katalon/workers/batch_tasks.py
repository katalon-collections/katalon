...
def _make_session() -> tuple[async_sessionmaker[AsyncSession], AsyncEngine]:
    # Imported here, not at module level: the test harness reloads
    # katalon.config per test session to pick up the ephemeral testcontainer
    # DB URL, and a module-level `from katalon.config import settings`
    # would keep a stale pre-reload Settings instance (see import_tasks.py,
    # which uses the same call-time-import convention for this reason).
...
def _queue_batch_notification(
    ...
    if result["errors"] and result["affected"] == 0:
        ...
        text = (
            f"Die Batch-Bearbeitung von {record_type}-Datensätzen konnte nicht ausgeführt werden."
        )
    else:
...
def batch_edit_task(
    ...
    async def _do() -> dict[str, Any]:
        async with AsyncSessionLocal() as session:
            # Re-check the acting user's current status and permissions at
            # execution time, not just at enqueue time (#389): a role
            # change, deactivation, or feature/record-permission revocation
            # between enqueue and execution must block further processing
            # instead of falling back to a looser async-only check.
            ...
            if user is None or not user.is_active:
                ...
                return {
                    "affected": 0,
                    "errors": ["Batch-Job abgebrochen: Benutzer nicht gefunden oder deaktiviert."],
                    "batch_job_id": batch_job_id,
                    "record_type": record_type,
                }
            if not await has_feature_permission(session, user, "batch"):
                self.update_state(
                    state="FAILURE", meta={"current": 0, "total": 0, "stage": "denied"}
                )
                return {
                    "affected": 0,
                    "errors": ["Batch-Job abgebrochen: Batch-Feature wurde entzogen."],
                    "batch_job_id": batch_job_id,
                    "record_type": record_type,
                }
            if not await has_record_permission(session, user, record_type, "update"):
                self.update_state(
                    state="FAILURE", meta={"current": 0, "total": 0, "stage": "denied"}
                )
                return {
                    "affected": 0,
                    "errors": [
                        "Batch-Job abgebrochen: Schreibrecht für diesen Datensatztyp wurde entzogen."
                    ],
                    "batch_job_id": batch_job_id,
                    "record_type": record_type,
                }
