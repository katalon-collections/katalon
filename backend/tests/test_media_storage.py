...
@pytest.mark.asyncio
async def test_local_storage_delete_raises_with_failing_key_but_still_removes_others(
    tmp_path, monkeypatch
) -> None:
    """A permission error (or any OSError besides missing-file) on one key must
    surface as StorageDeleteError naming that key, while every other key in the
    same call is still deleted — #390 requires per-object failure visibility so
    a caller can leave exactly the failing row pending retry."""
    monkeypatch.setattr(config.settings, "media_root", str(tmp_path))
    storage = LocalStorage()
    ok_path = storage_path("ab/ok.jpg")
    bad_path = storage_path("ab/bad.jpg")
    ok_path.parent.mkdir(parents=True, exist_ok=True)
    ok_path.write_bytes(b"pixels")
    bad_path.write_bytes(b"pixels")

    real_unlink = Path.unlink

    def _flaky_unlink(self, missing_ok=False):  # noqa: ANN001
        if self == bad_path:
    ...
    with pytest.raises(StorageDeleteError) as exc_info:
        await storage.delete("ab/ok.jpg", "ab/bad.jpg")
    assert set(exc_info.value.failures) == {"ab/bad.jpg"}
    monkeypatch.undo()
...
@pytest.mark.asyncio
async def test_s3_storage_delete_raises_with_per_object_errors_from_batch_response(
    monkeypatch,
) -> None:
    """DeleteObjects can partially fail — AWS reports per-key failures in the
    response's Errors list rather than raising. #390 requires this to surface
    as StorageDeleteError naming exactly the failing key(s), not be silently
    treated as a full success."""
    storage, stubber = _stubbed_s3(monkeypatch)
    stubber.add_response(
        "delete_objects",
        {
            "Deleted": [{"Key": "ab/ok.jpg"}],
            "Errors": [{"Key": "ab/bad.jpg", "Code": "AccessDenied", "Message": "Access Denied"}],
        },
        expected_params={
            "Bucket": "test-bucket",
            "Delete": {"Objects": [{"Key": "ab/ok.jpg"}, {"Key": "ab/bad.jpg"}]},
        },
    )
    with stubber:
        with pytest.raises(StorageDeleteError) as exc_info:
            await storage.delete("ab/ok.jpg", "ab/bad.jpg")
    assert set(exc_info.value.failures) == {"ab/bad.jpg"}
    assert "AccessDenied" in exc_info.value.failures["ab/bad.jpg"]
