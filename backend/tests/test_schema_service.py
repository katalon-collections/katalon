...
def make_group_field(
    name: str, sub_fields: list[MagicMock], *, is_required: bool = False
) -> MagicMock:
...
@pytest.mark.asyncio
async def test_configured_field_types_are_validated_without_rejecting_legacy_metadata() -> None:
    fields = [
        make_field("amount", field_type="number"),
        make_field("published", field_type="boolean"),
        make_field("keywords", field_type="vocab_free"),
        make_field("coordinates", field_type="geo"),
        make_field("subject", field_type="vocab"),
    ]
    ...
    errors = await validate_metadata(
        ...
        "object",
        {
            ...
            "keywords": {"not": "text"},
            "coordinates": 52.5,
            "subject": "not-a-vocabulary-reference",
            "legacy_field": {"kept": "without schema validation"},
        },
    )

    assert errors == [
        "Feld 'amount' muss eine Zahl enthalten.",
        "Feld 'published' muss einen Boolean enthalten.",
        "Feld 'keywords' muss Text enthalten.",
        "Feld 'coordinates' muss Text enthalten.",
        "Feld 'subject': Vokabularwert muss ein Objekt {id, label} sein.",
    ]


@pytest.mark.asyncio
async def test_group_subfields_use_the_same_type_validation() -> None:
    group = make_field("details", field_type="group")
    sub_fields = [
        make_field("amount", field_type="number"),
        make_field("published", field_type="boolean"),
        make_field("keywords", field_type="vocab_free"),
    ...
    errors = await validate_metadata(
        mock_db(group, sub_fields=sub_fields),
        "object",
        {
            "details": [
                {
    ...
    assert errors == [
        "Feld 'details.amount' (Eintrag 1): muss eine Zahl enthalten.",
        "Feld 'details.published' (Eintrag 1): muss einen Boolean enthalten.",
        "Feld 'details.keywords' (Eintrag 1): muss Text enthalten.",
        "Feld 'details.coordinates' (Eintrag 1): muss Text enthalten.",
        "Feld 'details.subject' (Eintrag 1): Vokabularwert muss ein Objekt {id, label} sein.",
    ]
...
async def test_prepare_metadata_drops_manual_pid_input_on_create() -> None:
    pid = make_pid_field()
    metadata = await prepare_metadata(
        mock_db(pid), "object", {"pid": {"value": "urn:x", "label": "x"}}
    )
...
@pytest.mark.asyncio
async def test_prepare_metadata_coerces_number_fields() -> None:
    num_field = make_field("count", field_type="number")
    rep_num = make_field("measurements", field_type="number", is_repeatable=True)
    group = make_field("details", field_type="group")
    sub_num = make_field("sub_count", field_type="number")

    metadata = await prepare_metadata(
        mock_db(num_field, rep_num, group, sub_fields=[sub_num]),
        "object",
        {
            "count": "10.000,65",
            "measurements": ["1.5", 42, "23,5", "", None],
            "details": [{"sub_count": "100"}],
        },
    )

    assert metadata["count"] == 10000.65
    assert metadata["measurements"] == [1.5, 42, 23.5]
    assert metadata["details"] == [{"sub_count": 100}]


@pytest.mark.asyncio
async def test_validate_metadata_accepts_valid_numbers_and_empty_strings() -> None:
    opt_num = make_field("count", field_type="number", is_required=False)
    req_num = make_field("required_count", field_type="number", is_required=True)

    # Empty optional number should pass
    errors = await validate_metadata(
        mock_db(opt_num),
        "object",
        {"count": ""},
    ...
    # Valid numeric strings (including German localized) should pass
    errors = await validate_metadata(
        mock_db(opt_num),
        "object",
        {"count": "10.000,65"},
    ...
    # 23-3 is invalid and must fail
    errors = await validate_metadata(
        mock_db(opt_num),
        "object",
        {"count": "23-3"},
    )
    assert errors == ["Feld 'count' muss eine Zahl enthalten."]
    ...
    # Empty required number should fail required check
    errors = await validate_metadata(
        mock_db(req_num),
        "object",
        {"required_count": ""},
    )
    assert errors == ["Feld 'required_count' ist erforderlich."]
