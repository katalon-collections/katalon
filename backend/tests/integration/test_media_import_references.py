...
from katalon.core.models import MediaImportReference, Object, User
...
async def test_unknown_media_selector_blocks_dry_run_and_import(
    ...
    dry_response = await async_client.post("/v1/importer/dry-run", headers=auth_headers, json=body)
...
async def test_existing_skip_stores_media_references_without_changing_metadata(
    ...
    async with AsyncSessionLocal() as session:
        ...
        admin_user_id = str(
            await session.scalar(select(User.id).where(User.email == "admin@katalon.dev"))
        )
...
async def test_manual_mapping_deduplicates_identical_rows_and_blocks_distinct_targets(
    ...
    (conflict_job / "mapping.csv").write_text(
        f"filename,object_id\nconflict.png,{first_object_id}\nconflict.png,{second_object_id}\n",
        encoding="utf-8",
    )
