...
@pytest.mark.asyncio
async def test_update_authority_field_ignores_now_disabled_source(
    async_client: AsyncClient, auth_headers: dict
) -> None:
    """Regression: toggling an unrelated property (is_facet) on an existing
    authority-type field must not fail just because its already-configured
    `settings.source` was disabled/removed after the field was created —
    the validation only re-checks the source when it actually changes."""
    create_r = await async_client.post(
        "/v1/schema",
        headers=auth_headers,
        json={
            "target_type": "object",
            "name": "test_authority_grandfather",
            "label": {"de": "Normdatum"},
            "field_type": "authority",
            "is_required": False,
            "is_repeatable": False,
    ...
    disable_r = await async_client.patch(
        "/v1/authorities/gnd", headers=auth_headers, json={"is_enabled": False}
    )
    ...
    assert update_r.json()["is_facet"] is True

    # A genuine source change to an unknown/unregistered source must still be rejected.
    reject_r = await async_client.put(
        f"/v1/schema/{field_id}",
        headers=auth_headers,
        json={
            "target_type": field["target_type"],
            ...
            "is_repeatable": field["is_repeatable"],
            "sort_order": field["sort_order"],
            "settings": {"source": "totally-unknown-source"},
        },
    )
    assert reject_r.status_code == 422, reject_r.text
