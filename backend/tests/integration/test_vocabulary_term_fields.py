...
@pytest.mark.asyncio
async def test_schema_rejects_invalid_regex_and_pid_pattern_settings(
    async_client: AsyncClient, auth_headers: dict
) -> None:
    invalid_regex = await async_client.post(
        "/v1/schema",
        headers=auth_headers,
        json={
            "target_type": "object",
            "name": f"invalid_regex_{uuid.uuid4().hex}",
            "label": {"de": "Ungültiger Regex"},
            "field_type": "text",
            "settings": {"validation_regex": "["},
        },
    )
    assert invalid_regex.status_code == 422, invalid_regex.text
    assert "validation_regex" in invalid_regex.json()["detail"]

    invalid_pid_pattern = await async_client.post(
        "/v1/schema",
        headers=auth_headers,
        json={
            "target_type": "object",
            "name": f"invalid_pid_pattern_{uuid.uuid4().hex}",
            "label": {"de": "Ungültiges PID-Pattern"},
            "field_type": "pid",
            "settings": {"pattern": "("},
        },
    )
    assert invalid_pid_pattern.status_code == 422, invalid_pid_pattern.text
    assert "pattern" in invalid_pid_pattern.json()["detail"]
...
async def test_vocabulary_canonical_uri_and_alignments(
    ...
    assert any(
        n["term"] == "monograph" and n["uri"] == "http://vocab.getty.edu/aat/300028051"
        for n in tree_nodes
    )
    ...
    imported_term = (
        await async_client.get(f"/v1/vocabularies/{vocab['id']}/terms", headers=auth_headers)
    ).json()
...
"""
    ...
    terms_empty = (
        await async_client.get(f"/v1/vocabularies/{vocab_id}/terms", headers=auth_headers)
    ).json()
...
@pytest.mark.asyncio
async def test_duplicate_vocabulary_term(async_client: AsyncClient, auth_headers: dict) -> None:
    vocab_response = await async_client.post(
        "/v1/vocabularies",
        headers=auth_headers,
