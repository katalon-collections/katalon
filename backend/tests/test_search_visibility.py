...
def test_index_doc_builds_numeric_facet_field() -> None:
    ...
    doc = search_service._build_doc(
        "object", obj, field_definitions=fields,
    )

    # facet_all_* is written for every public field, not just is_facet ones.
    assert doc["facet_all_year"] == ["1949", "1950"]
    assert doc["number_facet_all_year"] == [1949.0, 1950.0]
...
@pytest.mark.asyncio
async def test_search_documents_empty_query_defaults_to_idno_sort(monkeypatch) -> None:
    captured: dict = {}
    ...
    await elasticsearch.search_documents(None, "object", "public", 0, 20)

    assert captured["body"]["sort"] == [{"idno.keyword": "asc"}]


@pytest.mark.asyncio
async def test_search_documents_non_empty_query_has_no_forced_sort(monkeypatch) -> None:
    captured: dict = {}
    ...
    await elasticsearch.search_documents("marrakesch", "object", "public", 0, 20)
...
async def test_search_documents_filters_and_aggregates_numeric_facets(monkeypatch) -> None:
    ...
    body = captured["body"]
    selected_range = {"range": {"number_facet_all_year": {"gte": 1900.0, "lte": 1950.0}}}
    ...
    assert aggregation["aggs"]["filtered"]["aggs"]["values"] == {
        "stats": {"field": "number_facet_all_year"}
    }
...
@pytest.mark.asyncio
async def test_search_resolves_idno_subtitle_without_facet_all_field(monkeypatch) -> None:
    """`idno` has no `facet_all_idno` (it isn't a facet), so subtitle resolution
    must read it straight off the document like the record_type/status pseudo-fields."""
                ...
                "hits": [
                    {
                        "_id": "abc",
                        "_score": 1.0,
                        "_source": {"record_type": "object", "idno": "OBJ-42", "status": "public"},
                    }
                ],
            },
            "aggregations": {},
