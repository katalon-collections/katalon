...
def _make_session(media_file=None):
    ...
    session = AsyncMock()
    # AsyncMock auto-specs child attributes as AsyncMock too, so an
    # unconfigured `session.info` would make `.info.setdefault(...)` inside
    # workers.enqueue.after_commit() return a coroutine instead of a list.
    # Real AsyncSession.info is a plain dict — match that here.
    session.info = {}
