...
const RECORD_LABEL_KEYS: Record<string, string> = {
  object: 'sidebar.nav.list',
  collection: 'sidebar.nav.collectionsList',
  entity: 'sidebar.nav.entitiesList',
  place: 'sidebar.nav.placesList',
  occurrence: 'sidebar.nav.occurrencesList',
  procedure: 'sidebar.nav.proceduresList',
  storage_location: 'sidebar.nav.storageLocations',
}
...
function typeLabel(kind: string, language: string, t: TFunction): string {
  return RECORD_LABEL_KEYS[kind]
    ? t(RECORD_LABEL_KEYS[kind])
    : ADMIN_LABELS[kind]?.[language.startsWith('de') ? 0 : 1] ?? kind
}

function recordResult(result: SearchResult): AdminSearchResult {
  return {
    id: result.id,
    kind: result.record_type,
    title: result.title,
    subtitle: result.status,
    route: RECORD_ROUTES[result.record_type],
    edit_id: result.id,
  }
}
...
export function Topbar({ crumbs, route, onNavigate, currentUser, onLogout, onOpenNavigation }: Props) {
  ...
  function handleSelect(r: AdminSearchResult) {
    setQ('')
    setOpen(false)
    onNavigate?.(RECORD_ROUTES[r.kind] ?? r.route, r.edit_id ?? undefined)
  }
            ...
            {results.map(r => (
              ...
              >
                <span style={{ fontSize: 10, fontWeight: 600, background: 'var(--accent-50)', color: 'var(--accent-ink)', padding: '1px 5px', borderRadius: 4, flexShrink: 0 }}>
                  {typeLabel(r.kind, i18n.language, t)}
                </span>
