...
export function AppShell() {
  ...
  const [pendingNavigation, setPendingNavigation] = useState<{ route: string; id?: string | null } | null>(null)
  ...
  function safeNavigate(r: string, id?: string | null) {
    if (isDirtyRef.current) {
      setPendingNavigation({ route: r, id })
      return
    }
  ...
  function renderScreen() {
    switch (route) {
      ...
      case 'storage-locations': return features.includes('storage_locations') ? <ScreenStorageLocation initialLocationId={editId} onLocationSelect={(id) => navigate('storage-locations', id)} onOpenObject={(id) => navigate('form', id)} /> : <Placeholder label="Kein Zugriff" />
  ...
  return (
    <div className="app">
      ...
      {pendingNavigation && (
        <div className="batch-modal-backdrop" onClick={cancelLeave} role="dialog" aria-modal="true">
          <div className="batch-modal" onClick={e => e.stopPropagation()} style={{ width: 480 }}>
            <div className="batch-modal-header">
              <h2>{t('unsavedChangesModal.title')}</h2>
              <button type="button" className="btn sm ico gh" onClick={cancelLeave} aria-label="Schließen">
                <X size={16} />
              </button>
            </div>
