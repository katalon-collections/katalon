...
import { objects, entities, places, occurrences, procedures, collections, storageLocations, vocabularies } from '../../api/client'
...
export function BatchEditModal({ recordType, fields, selection, onClose, onSuccess }: Props) {
  ...
  const TYPE_LABELS: Record<RecordType, string> = {
    ...
    storage_location: t('types.storage_location'),
  }
  ...
  const hasStatusOptions = (STATUS_OPTIONS[recordType]?.length ?? 0) > 0
  const [opType, setOpType] = useState<BatchOperationType>(hasStatusOptions ? 'set_status' : 'set_field')
  ...
  function buildOperation(): BatchOperation {
    switch (opType) {
      case 'set_status':
        if (!hasStatusOptions) throw new Error(t('errors.unknownOperation'))
          ...
          <form id="batch-form" onSubmit={handleSubmit}>
            <div className="fld-row">
              ...
              <select className="fld" value={opType} onChange={e => setOpType(e.target.value as BatchOperationType)}>
                {hasStatusOptions && <option value="set_status">{t('operations.setStatus')}</option>}
            ...
            {opType === 'set_status' && hasStatusOptions && (
              <div className="fld-row">
