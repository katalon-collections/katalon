...
import { Alert, AlertCircle, Bookmark, Calendar, ChevD, ChevR, ListTree, Plus, Upload, X, Trash, Lightning, File, Music, Video, FileText, Box, Eye, Download } from '../ui/Icons'
...
function parseLocalizedNumber(raw: unknown): number | null {
  ...
  // Any remaining sign means invalid (e.g. "23-3")
  if (s.includes('-') || s.includes('+')) return null

  // Check scientific notation fallback (e.g. "1e-3")
  if (s.toLowerCase().includes('e') && !s.includes(',')) {
    const num = Number(sign + s)
    return Number.isFinite(num) ? num : null
  }

  // Must only contain digits, dots, commas
  ...
  if (hasDot && hasComma) {
    ...
    if (lastComma > lastDot) {
      ...
      const parts = s.slice(0, lastComma).split('.')
      if (!parts[0] || parts[0].length > 3 || !/^\d+$/.test(parts[0])) return null
      for (let i = 1; i < parts.length; i++) {
        if (parts[i].length !== 3 || !/^\d+$/.test(parts[i])) return null
      }
      const dec = s.slice(lastComma + 1)
      if (!/^\d+$/.test(dec)) return null
      const num = Number(sign + parts.join('') + '.' + dec)
      return Number.isFinite(num) ? num : null
    } else {
      // US notation: 10,000.65
      const parts = s.slice(0, lastDot).split(',')
      if (!parts[0] || parts[0].length > 3 || !/^\d+$/.test(parts[0])) return null
      for (let i = 1; i < parts.length; i++) {
        if (parts[i].length !== 3 || !/^\d+$/.test(parts[i])) return null
      }
    ...
    if (commaCount === 1) {
      ...
      const num = Number(sign + parts.join(''))
      return Number.isFinite(num) ? num : null
    }
  } else if (hasDot) {
...
function isValidNumberInput(raw: unknown): boolean {
  if (raw === '' || raw == null) return true
  return parseLocalizedNumber(raw) !== null
}
...
function NumberInput({
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === '-') {
      ...
      if (start === 0 && (!input.value.startsWith('-') || end > 0)) {
        return
      }
      e.preventDefault()
      return
  ...
  function handlePaste(e: React.ClipboardEvent<HTMLInputElement>) {
    ...
    const next = current.slice(0, start) + trimmed + current.slice(end)
    if (next.indexOf('-') > 0 || (next.match(/-/g) || []).length > 1) {
      e.preventDefault()
      return
    }
  }
...
function sanitizeMetadataForSave(
  ...
  for (const f of fields) {
    if (f.field_type === 'number') {
      ...
      if (f.is_repeatable && Array.isArray(v)) {
        result[f.name] = v
        ...
        result[f.name] = parsed !== null ? parsed : v
      }
    } else if (f.field_type === 'group' && Array.isArray(result[f.name])) {
      const arr = result[f.name] as Record<string, unknown>[]
      const groupSubFields = f.children ?? []
      const numSubFields = groupSubFields.filter(sf => sf.field_type === 'number')
      if (numSubFields.length > 0) {
        result[f.name] = arr.map(instance => {
          if (!instance || typeof instance !== 'object') return instance
          const cleanedInst = { ...instance }
          for (const sf of numSubFields) {
            const sv = cleanedInst[sf.name]
            if (sv === '' || sv == null) {
              cleanedInst[sf.name] = null
            } else if (typeof sv !== 'number') {
              const parsed = parseLocalizedNumber(sv)
              cleanedInst[sf.name] = parsed !== null ? parsed : sv
            }
          }
          return cleanedInst
        })
...
function VocabInput({ vocabId, value, onChange, disabled, autoFocus, onCancel }: {
  ...
  const [deletedTerm, setDeletedTerm] = useState(false)
  ...
  useEffect(() => {
    ...
    vocabularies.ancestors(vocabId, value.id)
      .then(ancestors => {
        if (current) setAncestorPath(ancestors.map(term => getLabel(term)).join(' › '))
      })
      .catch(() => {
        if (current) {
          setAncestorPath('')
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        <span style={{
          ...
          {deletedTerm && <span style={{ fontSize: 11, color: 'var(--danger)' }}>{t('vocabPicker.deletedTerm')}</span>}
        {!disabled && (
...
function QuickCreateDialog({
  ...
  return createPortal(
    ...
    >
      ...
      />
      {showDiscardConfirm && (
        <div className="batch-modal-backdrop" onClick={() => setShowDiscardConfirm(false)} role="dialog" aria-modal="true" style={{ zIndex: 10000 }}>
          <div className="batch-modal" onClick={e => e.stopPropagation()} style={{ width: 480 }}>
            <div className="batch-modal-header">
              <h2>{t('unsavedChangesModal.title')}</h2>
              <button type="button" className="btn sm ico gh" onClick={() => setShowDiscardConfirm(false)} aria-label="Schließen"><X size={16} /></button>
            </div>
            ...
            <div className="batch-modal-footer">
              ...
              <button type="button" className="btn danger" onClick={forceClose}>
...
export function ScreenForm({ recordType, recordId, onBack, onSaved, onDirtyChange, variantHint, quickCreate = false, initialSubtype, lockSubtype = false, initialLabel, onCreated, initialParentId }: Props) {
  ...
  async function handleDeleteRelation(id: string) {
    setConfirmState({
      message: 'Relation wirklich löschen?',
      ...
      case 'number':
        return (
          <NumberInput
            ...
            onChange={onChange}
            disabled={disabled}
            placeholder={t('numberInput.placeholder')}
          />
        )
      case 'date':
    ...
    for (const f of fields) {
      ...
      if (f.is_repeatable) {
        ...
        for (let i = 0; i < arr.length; i++) {
          ...
          if (f.field_type === 'number' && item !== '' && item != null) {
            if (!isValidNumberInput(item)) {
              errors[f.name] = 'Ungültige Zahl. Erlaubt: Ganze Zahlen und Dezimalzahlen (z.B. 42 oder 10.000,65)'
              break
            }
          }
  ...
  function validateSingleField(field: FieldDefinition): { level: 'error' | 'warning'; message: string } | null {
    if (field.name === 'idno') return null
    ...
    if (field.field_type === 'number') {
      if (!isValidNumberInput(val)) {
        return { level: 'error', message: 'Ungültige Zahl. Erlaubt: Ganze Zahlen und Dezimalzahlen (z.B. 42 oder 10.000,65)' }
      }
    }
  ...
  async function handleSave() {
    ...
    setFieldWarnings(validation.warnings)
    try {
      const cleanMetadata = sanitizeMetadataForSave(values, fields)
  ...
  async function commitMerge(
    ...
    setError(null)
    try {
      const cleanMerged = sanitizeMetadataForSave(mergedMetadata, fields)
      const finalPayload = { ...payload, metadata_: cleanMerged }
      ...
      setValues(cleanMerged)
      setBaseValues(cleanMerged)
      setVersion(updated.version)
  ...
  async function handleDeleteMedia(mediaId: string) {
    if (!savedId) return
    setConfirmState({
      message: 'Medium wirklich löschen?',
      confirmLabel: 'Löschen',
      danger: true,
      onConfirm: async () => {
        try {
  ...
  const legacyEntries = Object.entries(values).filter(([name]) => !knownFieldNames.has(name))
            ...
            <div className="card">
              ...
              <div className="bd">
                          ...
                          {(vals ?? []).map((v, i) => (
                            <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                              {f.field_type === 'date' ? (
                                ...
                                <NumberInput
                                  value={v}
                                  onChange={value => updateRepeat(f.name, i, value)}
                                  placeholder={t('numberInput.placeholder')}
                                  disabled={justCreated}
                                />
                              ) : (
                ...
                {legacyEntries.length > 0 && (
                  <section aria-label={t('legacy.heading')} style={{ marginTop: 16, borderTop: '1px solid var(--border-s)', paddingTop: 12 }}>
                    <div className="lbl">{t('legacy.heading')}</div>
                    {legacyEntries.map(([name, value]) => (
                      <div key={name} className="fg-2" style={{ alignItems: 'center', marginTop: 6 }}>
                        <code style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}: {JSON.stringify(value)}</code>
                        <button type="button" className="btn sm gh dn" onClick={() => {
                          setValues(current => {
                            const next = { ...current }
                            delete next[name]
                            return next
                        ...
                        <div key={snap.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--border)' }}>
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (!savedId) return
                              setConfirmState({
                                message: `Version „${snap.label}" wiederherstellen?`,
