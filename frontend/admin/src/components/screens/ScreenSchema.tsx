...
function FieldDetail({ form, availableFields, fieldId, isNew, saving, error, showSubtype, authoritySources, pidProviders, aiAvailable, onChange, onSave, onDelete, onDuplicate, onClose, onSubFieldChange, onRequestDeleteSubfield }: FieldDetailProps) {
  ...
  function handleSubFieldDelete(sf: FieldDefinition) {
    if (onRequestDeleteSubfield) {
      onRequestDeleteSubfield(sf)
      return
    }
    schema.delete(sf.id).then(() => onSubFieldChange()).catch(e => alert((e as Error).message))
  }
              ...
              <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 16 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input
                    type="checkbox"
                    className="ck"
                    checked={form.show_in_detail}
                    onChange={e => {
                      const checked = e.target.checked
                      onChange({ ...form, show_in_detail: checked, is_facet: checked ? form.is_facet : false })
                    }}
                  />
                ...
                <label
                  style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: form.show_in_detail ? 1 : 0.5 }}
                  title={form.show_in_detail ? undefined : t('fieldDetail.isFacetDisabledHint')}
                >
                  <input
                    type="checkbox"
                    className="ck"
                    checked={form.is_facet}
                    disabled={!form.show_in_detail}
                    onChange={e => set('is_facet', e.target.checked)}
                  />
              ...
              <div key={sf.id} style={{ fontSize: 13 }}>
                {subFieldEditing === sf.id ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '1px solid var(--border-s)' }}>
                    ...
                    <button className="btn sm ico gh dn" onClick={() => handleSubFieldDelete(sf)}><Trash size={12} /></button>
                  </div>
                )}
              </div>
            ))}
...
function DeleteFieldModal({
  ...
  async function handleConfirm() {
    ...
    setError(null)
    try {
      await schema.delete(field.id, {
        purgeData: usageCount !== null && usageCount > 0 ? purgeData : false,
      })
      onDeleted()
    } catch (err) {
      ...
      <div className="batch-modal batch-modal--compact" style={{ width: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="batch-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <p style={{ margin: 0, fontSize: 14 }}>
            {field.isSubfield
              ? t('deleteFieldModal.subfieldQuestion', { name: field.label })
              : t('deleteFieldModal.question', { name: field.label })}
          </p>

          {loadingUsage ? (
            <div style={{ fontSize: 13, color: 'var(--fg-3)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span className="spinner sm" />
              <span>{t('deleteFieldModal.checkingUsage')}</span>
            </div>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <strong style={{ display: 'block', marginBottom: 4 }}>
                    {t('deleteFieldModal.warningTitle')}
                  </strong>
                  <p style={{ margin: 0, fontSize: 13, color: '#92400e' }}>
                    {t(usageCount === 1 ? 'deleteFieldModal.warningUsed' : 'deleteFieldModal.warningUsed_plural', { count: usageCount })}
                  </p>
                </div>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
                {/* Option 1: Purge all data (Standard) */}
                <label
                  style={{
                    display: 'flex',
                    ...
                    borderRadius: 6,
                    background: purgeData ? 'var(--panel-active, rgba(0,0,0,0.03))' : 'var(--bg-1, #fff)',
                    cursor: 'pointer',
                  }}
                >
                  <input
                    ...
                    value="purge"
                    checked={purgeData}
                    onChange={() => setPurgeData(true)}
                    style={{ marginTop: 3 }}
                  />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: '#b91c1c' }}>
                      {t('deleteFieldModal.optionPurgeData')}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--fg-2)', marginTop: 2 }}>
                      {t(usageCount === 1 ? 'deleteFieldModal.optionPurgeDataDesc_one' : 'deleteFieldModal.optionPurgeDataDesc', { count: usageCount })}
                    </div>
                  </div>
                </label>

                {/* Option 2: Keep historical data */}
                <label
                  style={{
                    display: 'flex',
                    alignItems: 'flex-start',
                ...
                >
                  <input
                    ...
                    value="keep"
                    checked={!purgeData}
                    onChange={() => setPurgeData(false)}
                    style={{ marginTop: 3 }}
                  />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                      {t('deleteFieldModal.optionKeepData')}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--fg-2)', marginTop: 2 }}>
                      {t('deleteFieldModal.optionKeepDataDesc')}
                    </div>
                  </div>
                </label>
              </div>
        ...
        <div className="batch-modal-footer">
          <button className="btn" onClick={onClose} disabled={deleting}>
            {t('deleteFieldModal.cancel')}
          </button>
          <button className="btn danger" onClick={handleConfirm} disabled={deleting || loadingUsage}>
            {deleting
              ? t('deleteFieldModal.deleting')
              : usageCount !== null && usageCount > 0
                ? purgeData
                  ? t('deleteFieldModal.confirmButtonPurge')
                  : t('deleteFieldModal.confirmButtonKeep')
                : t('deleteFieldModal.confirmButton')}
          </button>
        </div>
      </div>
    </div>
...
export function ScreenSchema({ initialPath, onPathChange }: Props = {}) {
  ...
  function handleFieldDeleted() {
    if (deleteTarget) {
      ...
      const wasSubfield = deleteTarget.isSubfield
