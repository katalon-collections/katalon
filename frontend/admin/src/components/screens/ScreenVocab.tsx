...
import { ConflictError, schema, vocabularies } from '../../api/client'
...
import { Alert, ChevD, ChevR, Copy, Edit, Help, Plus, Tag, Trash, Upload, X } from '../ui/Icons'
...
function DeleteTermModal({
  ...
  useEffect(() => {
    let active = true
    vocabularies.getTermUsage(term.id)
      .then(res => {
        if (active) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (err instanceof ConflictError) {
        setUsageCount(err.related_count)
      }
  ...
  return (
    <div className="batch-modal-backdrop" onClick={onClose} role="dialog" aria-modal="true">
      <div className="batch-modal" onClick={e => e.stopPropagation()} style={{ width: 540 }}>
        <div className="batch-modal-header">
          <h2>{t('termDeletion.title')}</h2>
          <button type="button" className="btn ico gh" onClick={onClose} aria-label="Schließen">
            <X size={16} />
          </button>
        </div>
        <div className="batch-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <p style={{ margin: 0, fontSize: 14 }}>
            {t('termDeletion.confirmPrompt', { term: termLabel })}
          </p>
              ...
              <div className="batch-warning">
                ...
                <div style={{ flex: 1 }}>
                  <strong style={{ display: 'block', marginBottom: 4 }}>
                    {t('termDeletion.inUseWarningTitle')}
                  </strong>
                  <p style={{ margin: 0, fontSize: 13, color: '#92400e' }}>
                    {t(usageCount === 1 ? 'termDeletion.inUseBanner' : 'termDeletion.inUseBanner_plural', { count: usageCount })}
                  </p>
                </div>
              </div>

              <div>
                <p style={{ margin: '4px 0 8px', fontSize: 13, fontWeight: 500 }}>
                  {t('termDeletion.chooseAction')}
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {/* Option 1: Remove from all records (Default) */}
                  <label
                    style={{
                      display: 'flex',
                      ...
                      border: '1px solid var(--border)',
                      borderRadius: 6,
                      background: deleteAction === 'remove' ? 'var(--panel-active, rgba(0,0,0,0.03))' : 'transparent',
                      cursor: 'pointer',
                    }}
                  >
                    <input
                    ...
                    />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{t('termDeletion.optionRemove')}</div>
                      <div style={{ fontSize: 12, color: 'var(--fg-2)', marginTop: 2 }}>
                        {t('termDeletion.optionRemoveDesc', { count: usageCount })}
                      </div>
                    </div>
                  </label>
                  ...
                  <label
                    style={{
                      ...
                      border: '1px solid var(--border)',
                      borderRadius: 6,
                      background: deleteAction === 'force' ? 'var(--panel-active, rgba(0,0,0,0.03))' : 'transparent',
                      cursor: 'pointer',
                    }}
                  >
                    <input
                    ...
                    />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{t('termDeletion.optionForce')}</div>
                      <div style={{ fontSize: 12, color: 'var(--fg-2)', marginTop: 2 }}>
                        {t('termDeletion.optionForceDesc')}
                      </div>
                    </div>
                  </label>
                </div>
        ...
        <div className="batch-modal-footer">
          <button type="button" className="btn" onClick={onClose} disabled={submitting}>
            {t('termDeletion.cancel')}
          </button>
          <button type="button" className="btn danger" onClick={handleConfirm} disabled={submitting || loadingUsage}>
            {submitting ? t('termDeletion.deleting') : t('termDeletion.deleteButton')}
          </button>
        </div>
      </div>
    </div>
          ...
          {vocab && (
            <>
              ...
              {termToDelete && (
                <DeleteTermModal
                  term={termToDelete}
                  childCount={terms.filter(tItem => tItem.parent_id === termToDelete.id).length}
                  availableTerms={terms.filter(tItem => tItem.id !== termToDelete.id)}
