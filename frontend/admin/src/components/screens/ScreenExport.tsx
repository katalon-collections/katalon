  ...
  return (
    <div className="settings-page">
      <div className="settings-head" style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>Export</h1>
      </div>

      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        <button type="button" className={`btn sm ${tab === 'dumps' ? 'pri' : 'gh'}`} onClick={() => setTab('dumps')}>Dumps</button>
        <button type="button" className={`btn sm ${tab === 'mapping' ? 'pri' : 'gh'}`} onClick={() => setTab('mapping')}>Format-Mapping</button>
      </div>
  ...
  return (
    <div className="settings-card" style={{ maxWidth: 480 }}>
      ...
      {subtypeOptions.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <label style={lbl}>Subtyp (optional)</label>
      ...
      >
        <Download size={14} /> {downloading ? 'Exportiere…' : 'Export starten'}
      </button>
    </div>
  )
}
...
function MappingTab({ recordType }: { recordType: string }) {
  ...
  if (loading) return <div style={{ color: 'var(--fg-3)', fontSize: 13 }}>Lade…</div>
  if (fields.length === 0) return <div style={{ color: 'var(--fg-3)', fontSize: 13 }}>Keine mappbaren Felder für diesen Typ.</div>
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
        <thead>
          <tr>
            <th style={{ textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid var(--border)' }}>Feld</th>
            {formats.map(f => (
