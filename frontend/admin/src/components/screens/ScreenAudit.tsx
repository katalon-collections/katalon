...
import { MOCK_AUDIT } from '../../api/mock-data'
import { Check, Plus, Edit, Trash, Globe } from '../ui/Icons'
...
const USER_NAMES: Record<string, string> = { u1: 'M. Bauer', u2: 'T. Hofer', u3: 'S. Egli' }

export function ScreenAudit() {
  const [filter, setFilter] = useState<string>('all')

  const items = filter === 'all' ? MOCK_AUDIT : MOCK_AUDIT.filter(e => e.action === filter)
        ...
        {items.map(evt => {
          ...
          return (
            <div key={evt.id} className={`evt ic-${evt.action}`}>
              ...
              <div className="body">
                <div className="ti">
                  {ACTION_LABELS[evt.action]}: <b>{evt.record_id}</b>
                </div>
                  ...
                  <div className="diff">
                    {Object.entries(diff.old).map(([k, v]) => (
                      <div key={k}>
                        ...
                        <span className="rem">{v}</span>
                        {' → '}
                        <span className="add">{(diff.new ?? {})[k]}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="who">
                <div className="av">{(USER_NAMES[evt.user_id ?? ''] ?? '?')[0]}</div>
                {USER_NAMES[evt.user_id ?? ''] ?? '—'}
              </div>
            </div>
          )
        })}
