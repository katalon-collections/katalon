...
export function ScreenWorkingSets({ initialSetId, onOpenRecord, onSetIdChange }: Props) {
  ...
  const [confirmState, setConfirmState] = useState<{ message: string; confirmLabel: string; onConfirm: () => void | Promise<void> } | null>(null)
  ...
  const handleDeleteSet = (s: WorkingSet) => {
    setConfirmState({
      message: t('deleteSetConfirm', { name: s.name }),
