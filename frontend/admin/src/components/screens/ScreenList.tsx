...
export function ScreenList({ recordType, onOpen, initialTab, onTabChange }: Props) {
  ...
  async function handleDelete(id: string) {
    setConfirmState({
      message: t('deleteConfirm', { type: typeSingularLabels[recordType] }),
      onConfirm: async () => {
        try {
          await api.delete(id)
          removeLocally(id)
  ...
  const [lockMap, setLockMap] = useState<Record<string, LockInfo>>({})
  useEffect(() => {
    const ids = items.map(r => r.id)
    if (ids.length === 0) { setPresenceMap({}); setLockMap({}); return }
    presence.batch(recordType, ids).then(setPresenceMap).catch(() => {})
    locks.batch(recordType, ids).then(setLockMap).catch(() => {})
  }, [items, recordType])
